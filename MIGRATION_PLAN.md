...
| **페이로드** | kinematic_sync (기본) + optional FixedJoint | Gazebo DetachableJoint 대체. `docs/PAYLOAD_ATTACHMENT.md` |
...
#### Step 5: 페이로드 드롭 메커니즘 (구현됨)

1. 기본 `kinematic_sync`: 부착 시 pose/velocity 동기화 + 중력 비활성화; 투하 시 `detach_payload()`
2. 옵션 `fixed_joint`: 소수 env smoke용 USD FixedJoint (실패 시 kinematic 폴백)
3. Auto-drop: CCIP d_impact <= 0.5m
4. 착지 z <= 0.04m → `drop_error_actual` 텐서 + Layer 4 reward
5. **Gazebo drop_calculator 대체**: `env.drop_error_actual` + termination 인라인
