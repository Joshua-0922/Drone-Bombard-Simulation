...
def approach_distance_reward(
    ...
    if hasattr(env, "_last_rew_dist"):
        env._last_rew_dist[:] = reward.detach()
    return reward
...
def drop_accuracy_reward(
    ...
    total = landed.float() * (base_reward + jackpot)
    if hasattr(env, "_last_rew_drop"):
        env._last_rew_drop[:] = total.detach()
    return total


def drop_instability_penalty(
    env: ManagerBasedRLEnv,
    penalty: float = -50.0,
    limit_ang_vel: float = 2.0,
    limit_tilt: float = 0.26,
) -> torch.Tensor:
    """투하 착지 시 드론 불안정 패널티 (Gazebo Layer 4 instability)."""
    payload = env.scene["payload"]
    landed = env.dropped & (payload.data.root_pos_w[:, 2] <= 0.04)
    if not landed.any():
    ...
    drone = _get_drone(env)
    omega_mag = torch.norm(drone.data.root_ang_vel_w, dim=-1)
    ...
    unstable = (
        (omega_mag > limit_ang_vel)
        | (roll.abs() > limit_tilt)
        | (pitch.abs() > limit_tilt)
    )
    return (landed & unstable).float() * penalty


def physics_glitch_penalty(
    env: ManagerBasedRLEnv,
    penalty: float = -100.0,
    max_dist: float = 500.0,
) -> torch.Tensor:
    """물리 폭발 시 즉시 대형 패널티 (Gazebo physics_glitch)."""
    pos = _get_drone(env).data.root_pos_w[:, :2]
    diff = pos - env.target_pos
    d_xy = torch.norm(diff, dim=-1)
    is_glitch = ~torch.isfinite(d_xy) | (d_xy > max_dist)
    return is_glitch.float() * penalty
...
def energy_penalty(
    ...
    """급격한 기동 억제: 액션 크기 패널티."""
    action = env.action_cur
    return -w_energy * (action * action).sum(dim=-1)
