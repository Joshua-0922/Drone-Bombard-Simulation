...
from .payload_attachment import detach_payload, reattach_payload_on_reset, update_payload_attachment
...
def reset_episode_buffers(
    ...
    DroneBombardEnv 에 정의된 버퍼:
      ...
      - action_cur       [N, 5]   → 0
    """
    reattach_payload_on_reset(env, env_ids)
    env.action_prev[env_ids] = 0.0
    env.action_cur[env_ids] = 0.0
    if hasattr(env, "drop_error_actual"):
        env.drop_error_actual[env_ids] = float("nan")
    if hasattr(env, "d_impact_at_drop"):
        env.d_impact_at_drop[env_ids] = float("nan")
...
def check_ccip_auto_drop(env: ManagerBasedRLEnv, auto_drop_threshold: float = 0.5) -> None:
    ...
    if should_drop.any():
        drop_ids = should_drop.nonzero(as_tuple=False).squeeze(-1)
        if hasattr(env, "d_impact_at_drop"):
            env.d_impact_at_drop[drop_ids] = d_impact[drop_ids]
        detach_payload(env, drop_ids)
