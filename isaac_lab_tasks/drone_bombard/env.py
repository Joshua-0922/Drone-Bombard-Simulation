...
from .mdp.events import check_ccip_auto_drop, update_payload_attachment
...
class DroneBombardEnv(ManagerBasedRLEnv):
    ...
    def _pre_physics_step(self, action: torch.Tensor) -> None:
        ...
        # CCIP 자동 투하 체크
        check_ccip_auto_drop(self, auto_drop_threshold=0.5)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if done.any():
            ...
            for idx in done_ids.tolist():
                # payload_landed termination 이 성공으로 간주
                # (drop_accuracy reward > 0 이면 성공)
                payload = self.scene["payload"]
                d_error = torch.norm(
                    payload.data.root_pos_w[idx, :2] - self.target_pos[idx],
                    dim=-1,
                ).item()
    ...
    def _get_extras(self) -> dict[str, Any]:
        ...
        extras["env/payload_drop_rate"] = self.dropped.float().mean().item()
        extras["env/curriculum_stage_mean"] = self.curriculum_stage.float().mean().item()
        return extras
