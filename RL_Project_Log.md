...
09-02 축 P의 원인은 tilt 누적의 **접두(prefix) 평균**이었다. 에피소드 *전체* 평균이면
$\bar a \approx 0$이라 게인이 저절로 소거되지만, 정책이 들고 다니는 접두 평균에서는
$\overline{a}_{prefix} = (v_t-v_0)/t \neq 0$ 이 남아 $K_{policy}$ 오염원이 된다.
그 항을 2채널로 따로 주면 신경망이 뺀다 (회귀기 입력 36 → 38).
