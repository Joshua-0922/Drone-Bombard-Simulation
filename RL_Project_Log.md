...
**2026-09-25 (갱신) — 게이트 인지 학습 종료: exp_040 GRU-C(λ=20, 1000 epoch, EMA 없음) 12조건 × 3 seed 전부 동률(paired CI 0 포함) → 본 방법 = GRU-S + EMA 유지, 표·그림 변경 없음, 평활 항은 논문·코드에서 제외(롤백 완료). 개요 v6를 GRU-S 단일 방법으로 정리(§4.4 분산 수축 삭제). 다음: sim2real.**
...
- **2026-09-25:** exp_038 평활 항(λ 5/20/50, 27 run) ✅ EMA 대체 가능 · exp_039 게이트 손실/게이트 가중(9 run) ⛔ 이득 없음 · exp_040 GRU-C 최종 비교(36 run) 🟰 12/12 동률 → 본 방법 = GRU-S + EMA 유지.
