...
⛔ 튜닝 baseline 상대로는 "CEP50 격차가 모형오차와 함께 커진다"가 **L0에서 성립하지 않는다.**
✅ 살아남는 형태 둘: ① 잔차를 넣으면 부호가 뒤집힌다 → 주 주장은 **L1-SL로** 쓴다,
② 성공률·배달률 축에서는 격차가 실제로 커진다(배달률 +16.2 → **+28.2 pp**,
succ@0.5 +21.5 → **+26.2 pp**). DR {0, 0.5, 1.0} 27런 측정 중.
...
| # | 무엇 | 왜 | 비용 | 상태 |
|---|---|---|---|---|
| **1** | DR 축 27런 완료 → **그림 4 재작성** | 주 주장의 그림이 **미튜닝 baseline** 상대로 그려져 있다 | 진행 중 | 🔄 |
| **2** | 중간보고서 작성 (Table 1 + 그림 2·4·9·10) | 시드 정렬된 표가 확보됐다 | ~2시간 | ❌ |
