...
(정상 0.207 / 현실 A 0.204 / B 0.215 vs 0.204 / 0.201 / 0.219). exp_039: soft 첫 교차 게이트 손실은 악화(릴리즈 R² −0.148), 게이트 가중 MSE는 유의차 없음.
(→ exp_040에서 동률 확인, 위 갱신 참조.)
→ [[notes/experiments/exp_038_gate_aware_stage1]] · [[notes/experiments/exp_039_gate_aware_stage23]]

**2026-09-24 밤 — 최종 표·그림 재생성 완료 (본 방법 = L0 + GRU 잔차, GRU-S).** [[notes/research/final_tables_v6]]: Table 1(정상 바람, 0.204 / 0.403 / 90.8%, 오라클 이득 86%),
DR 스윕(0 → 2.5: +8.8% → −39.4%, 부호 반전 0.5~1.0), 바람 조건(현실 A/B −33%/−28%, 스트레스 사다리), 미지 사거리(−29.4%), ablation.
그림 3장 `notes/figures/`. GRU-S vs GRU-U rel p2는 paired 부트스트랩 전 조건 동률 → 단순한 GRU-S 채택. 남은 일: 비전 절, 플랫폼, 본문.

**2026-09-24 저녁 — exp_037: 현실 돌풍에서는 이득이 유지된다 → 본 방법 = GRU-S 확정.** 새 현실 바람 모드(평균풍 + Dryden형 OU 돌풍
20~30%, τ 10·3 s)에서 GRU-S −33% / −28%, 오라클 −36% / −35%; 모든 학습 팔이 정상 바람 이득 유지. τ 사다리의 τ ≤ 3 손해는 돌풍만 둔
스트레스 시험이었다(Rule 46). 상대 분산 수축은 불채택(분산의 τ 식별력 19%). 실현 라벨·분산 헤드는 스트레스 조건의 보조 변형.
→ [[notes/experiments/exp_037_relative_shrink_realistic_wind]] · [[notes/research/paper_outline_v6]]

**2026-09-24 — exp_036 분산 헤드: 시변 바람용 단일 모델 확보(GRU-U).** 실현 라벨 GRU에 log 분산 출력을 더해 NLL로 학습, 주입 시
$\sigma_0^2/(\sigma_0^2+\sigma^2)$ 수축. 같은 데이터·라벨의 GRU-R을 전 τ에서 이김: 정상 0.210, τ=10 **0.237 (−13.1%, 오라클+EMA −14.1%)**,
τ=3 +4.8%, τ=1 +9.0%(gi 동률), τ=0.3 +18.5%. 분산의 τ 식별력 19%라 τ ≤ 3 손해는 절반만. 후속 = 상대 분산 수축(재학습 없음).
→ [[notes/experiments/exp_036_uncertainty_head]]

**2026-09-24 — exp_035 실현 라벨: 옳은 방향(τ=10 −11.0%, 정상 0.202 유지), τ ≤ 3은 아직 손해.**
입력·모델·데이터 동일, 정답만 "낙하 중 실제 바람 적분"으로. 같은 조건의 순간 라벨 팔보다 전 τ에서 개선(τ=3 +14.8 → +7.0%,
τ=1 +29 → +17%) → Rule 45 처방 확인. 남은 손해의 원인은 회귀기가 τ를 몰라 혼합 평균 수축률을 쓰는 것 → 다음 = 분산 헤드
(가우시안 NLL, 분산 반비례 수축). → [[notes/experiments/exp_035_realised_label]]
...
**⛔ 09-14 exp_033 (std 고정, `tog7jqs5`) — 110 iter 중단, 음성 2.** 잔차는 0.06 → 0.19 m로 자랐지만 참 드리프트와
**반대 방향**(R² −0.04 → −0.48, 릴리즈 편향 +0.17 vs 참 −0.086 m). model_100 paired n=600: **CEP50 0.361 (+18.2%)**,
succ@0.5 65.67%(L0 78.83%). 보상 자체가 오르지 않음. 가설: 첫 교차 지점에서 착지 오차는 릴리즈 시점에 2차로 평탄한데
시간 비용 절감은 1차 → **릴리즈 트리거를 통한 시간 비용 farming**. **두 팔로 Rule 44 확정: 결과 공간 잔차의 학습 신호는
지도여야 한다.** → [[notes/experiments/exp_033_tog7jqs5_l1rl_fixed_std_pilot]]
**✅⛔ 09-14 오후 exp_034 — 학습된 시간 필터.** 원 관측 26채널 위의 GRU(64)가 수제 누적 12채널 없이 정상 바람 CEP50
**0.204 / CEP90 0.403 / succ@0.5 90.8%** (gi 0.209 / 0.411 / 89.7%, n=600) → **수제 특징 대체 성립.** 그러나 OU 데이터를 섞어
학습한 팔(GRU-M·MLP38-M)은 τ ≤ 3에서 정상 학습판보다 **더** 진다(τ=1: +29% / +20% vs gi +10%) — 라벨(릴리즈 순간 바람 고정 적분)이
순간 추종을 가르치고 그것이 오라클의 실패 방식이기 때문. **Rule 45: 시변 외란에서는 라벨이 병목.** 다음 = exp_035 실현 착지
드리프트 라벨(반사실, 기록된 미래 바람열로 적분). → [[notes/experiments/exp_034_learned_temporal_filter]]
**main claim 결정(사용자·09-14):** 결과 공간 잔차 지도학습. novelty = 주입 지점의 세 귀결(지도 가능·순항 유지·진단 가능).
RL은 ablation 한 문단. 행동 공간 잔차 RL은 별도 질문(사용자 결정 대기).
...
**✅ 정책 결합 해소 — 접두 평균 가속 (gi) 2채널**

09-02 축 P의 원인은 **누적 자세에 섞인 기동 성분**이었다. 누적 자세는
$\bar\theta = \theta_{wind} + \theta_{man}$ 이고, 앞쪽($F_{wind}/mg$)은 힘 균형이라
**컨트롤러와 무관**하지만 뒤쪽($\bar a_{prefix}/g$)은 **얼마나 공격적으로 가속하는가**에 달렸다.
에피소드를 완주하면 $\bar a \approx 0$이라 저절로 상쇄되지만, 잔차는 도중에 판단하므로
접두 평균을 쓰고 그때는 $\bar a_{prefix} = (v_t-v_0)/t \neq 0$ 이다.
그 항을 2채널로 따로 주면 신경망이 $\bar\theta - \bar a/g$ 를 만들어 뺀다 (입력 36 → 38).

> ⚠️ 09-02에 적었던 *"$\bar\theta \approx F_{wind}/K_{policy}$, 회귀기가 게인을 흡수한다"*
> 는 **메커니즘이 틀렸다.** 게인이 정하는 것은 기울기가 아니라 속도 추종 오차다.
> → [[notes/research/residual_policy_coupling]] §2

| seed-2 정책에 주입 (n=600) | succ@1.0 | succ@0.5 | CEP50 | CEP90 | 회수율 |
|---|---|---|---|---|---|
| L0 seed 2 (잔차 없음) | 96.00% | 84.67% | 0.274 | 0.542 | — |
...
자기 정책(seed-1)에서도 손해가 없고 꼬리 회수율이 64% → **90%**로 오른다.
→ **논문 Limitations에서 이 항목을 뺀다.**

오프라인으로 먼저 판정했다(GPU 0분). 덤프 파일 단위 홀드아웃(`--test_npz`):
tilt 0.611 → 0.554(격차 −0.057) vs **gi 0.668 → 0.679(격차 소멸)**.

**✅ Table 1 시드 정렬 — 철회했던 파레토 지배가 잔차와 함께 복귀**
...
무학습 팔을 4000·5000에서 재비행해(23 run, FAIL 0) 모든 행을 동일한 600 시나리오로 맞췄다.
...
**L0 단독으로는 여전히 CEP50에서 진다.** 차이를 만든 것이 정확히 잔차다 — A1 ablation의
가장 강한 형태다.

**⚠️ 새 규칙 (Rule 41) — $R^2$로 잔차를 통과시키지 마라**

전이 arm의 오프라인 $R^2$는 −0.057밖에 안 떨어졌는데 주입 회수율은 85% → 22.9%로
무너졌다. 손상이 분산이 아니라 **계통 편향·꼬리**에 있고, 첫 교차 게이트가 증폭한다.
채택 판정은 반드시 주입해서 한다(3 seed × 200 ep ≈ 90초).

**⚠️ 새로 열린 부채 — 그림 4를 다시 그려야 한다**

주 주장의 DR 스윕 그림이 **투하 고도 미튜닝(6.0 m) T2**를 상대로 그려져 있었다.

| DR | T2 **튜닝** CEP50 | L0 | **L1-SL** | T2−L0 | **T2−L1SL** |
|---|---|---|---|---|---|
| 1.5 | 0.282 | 0.305 | **0.209** | **−0.023** | **+0.073** |
| 2.5 | 0.405 | 0.498 | **0.334** | **−0.093** | **+0.071** |

**✅ DR 축 27런 완료 (FAIL 0) — 그림 4 확보, 주 주장이 더 강한 형태로 성립**

| DR | T2 **튜닝** | L0 | **L1-SL gi** | **T2 − gi** | **잔차 효과 (L0→gi)** | (abl) tilt |
|---|---|---|---|---|---|---|
| **0** | **0.082** | 0.125 | 0.129 | **−0.047** | **+3.7% — 해롭다** | +13.3% |
| 0.5 | **0.116** | 0.142 | 0.138 | **−0.022** | **−2.3%** | +6.7% |
| 1.0 | 0.195 | 0.208 | **0.171** | **+0.024** | −17.4% | −15.9% |
| **1.5** | 0.282 | 0.305 | **0.209** | **+0.073** | −31.6% | −31.4% |
| **2.5** | 0.405 | 0.498 | **0.310** | **+0.095** | −37.6% | −32.9% |

**단조 증가, 부호 전환 gi ≈ 0.35 / tilt ≈ 0.75.** 주 주장은 **L0가 아니라 L1-SL로** 쓴다.
⭐⭐ **잔차 자체가 DR 0에서 해롭다** — 보정할 편향이 없으면 분산만 더한다.
*"우리 방법이 낫다"* 가 아니라 **용량-반응 곡선이고 0 용량에서 0을 지나 음수로 간다.**
**두 특징 집합이 독립적으로 같은 모양**을 내는 것이 우연 가설을 더 강하게 배제한다.
⭐ **gi에서는 포화가 사라진다** — T2와의 격차가 +0.073 → **+0.095**로 계속 벌어진다.
세 축 전부 단조: CEP50 · succ@0.5 (+6.2 → **+31.0 pp**) · 배달률 (+7.1 → **+28.4 pp**).

**✅ gi를 본 방법으로 승격 (15 run, FAIL 0).** 모든 지표·조건에서 tilt보다 낫고,
**개선폭이 외삽에서 가장 크다** — DR 2.5 회수율 78% → **89.5%**, 미지 사거리
85.3% → **92.6%**, DR 1.5 CEP90 회수 64% → **90.5%**.

**✅ 채널 그룹 기여도 분해 (leave-one-group-out).** tilt 누적 −0.124 · gi −0.057 ·
나머지 5그룹 합 −0.028. **누적 12채널이 손실의 87%.** 자세를 통째로 빼도 **−0.001**
— tilt 누적이 이미 그 평균을 담고 있어 순간값은 잉여다. **정보는 순간이 아니라 누적에 있다.**

**⛔⛔ 마지막 미검증 전제를 닫았다 — 그리고 한계가 실재한다 (OU 바람, 60 run)**
...
1. **비행은 멀쩡하다** — L0 0.305 → 0.272~0.294, 배달률 95%. 깨지는 것은 **조준뿐**.
2. **매끄러움이 아니라 정보의 문제** — 원본 오라클은 τ=10에서도 진다(CEP90 1.233).
   EMA로 −14.1% 회복되지만 **τ≤3에서는 EMA를 걸어도 진다.**
3. ⭐⭐⭐ **참 바람을 아는 팔이 같은 지점에서 진다.** 착탄점은 낙하 0.85 s 구간의
   바람 적분인데 오라클은 릴리즈 순간의 바람만 안다 — 나머지는 **미래**다.
   **관측이 아니라 예보의 문제**이므로 이 한계는 방법의 흠이 아니라 **문제의 성질**이다.

$\tau \gtrsim 10\,t_{fall} \approx 8.5$ s (Rule 42). gi가 세 번째 축에서도 덜 무너진다
(τ=0.3 성공률 80.17% ≈ L0 80.50%, tilt 71.50%).

⚠️ **부수 — 상한선 팔에도 평활을 걸어야 한다.** 원본 오라클은 변하는 외란에서 천장 노릇을
못 한다. 상수 바람 대조에서 EMA는 0.192 → 0.188이라 **기존 천장 수치는 유효**.

**📄 [[notes/research/research_architecture]] v4 → v5** — §7 전면 갱신, §7.6(L1-SL 경로)·
§9.4(주 결과의 학습 프로토콜은 PPO가 아니다) 신설.
...
*(⚠️ 이 메커니즘 설명은 09-07에 정정됨 — 실제 오염원은 게인이 아니라 **기동 성분**이다.
→ [[notes/research/residual_policy_coupling]] §2)*
...
## 2026-09-07 (오후) — 중간보고서 Isaac Lab 파트 작성
...
| 결정 3 | 별도 ablation 절 없음 — 분석 절이 사실상 전부 ablation이므로 절 제목으로 흡수 |
| ⚠️ 정정 | *"동결 후 2단계 학습"* 은 **구현된 적이 없다.** L0는 단일 단계 PPO이고 "동결"은 L1-SL 데이터 수집(추론 전용)에만 해당 (08-29 §1.2의 재확인) |
| ⚠️ 미해결 | **1장에 안 들어간다.** 1장 = 본문 약 1,750자(한글 10pt·160%)인데 요약본이 3,074자. 문장 압축은 바닥이고 남은 절단 후보가 전부 필수 항목이라 **복수를 버려야** 1.0장이 된다 |
| 남은 것 | 그림 1·2 실제 생성(현재 캡션·축 사양만) · 분량 결정 · L0 학습 wall-clock 실측 |


## 2026-09-07 — exp_030: 게인 무관 특징 + Table 1 시드 정렬

| 항목 | 내용 |
|---|---|
| 실행 | 주입 평가 6 run + 오프라인 적합 3회 + 무학습 재측정 23 run (FAIL 0) + DR 축 27 run(진행 중) |
| 코드 | `_fit_sl_residual.py --gi/--test_npz` · `play.py` gi 채널 · `_t_reseed.sh` · `_gi_eval.sh` · `_dr_axis.sh` · `_agg_table1.py` |
| 결론 1 | **정책 결합 해소** — 전이 회수율 22.9% → **77.1%**, CEP90 0.830 → **0.464** |
| 결론 2 | **Table 1 시드 정렬** — L1-SL이 튜닝 T2를 **전 축 지배** (L0 단독으로는 CEP50 패배) |
| 결론 3 | **Rule 41** — $R^2$는 잔차의 통과 기준이 될 수 없다 |
| 부채 | 그림 4를 튜닝 baseline 상대로 다시 그려야 한다 (§7.4b.1) |
...
| # | 무엇 | 왜 | 비용 | 상태 |
|---|---|---|---|---|
| ~~1~~ | ~~DR 축 27런 → 그림 4~~ · ~~gi 헤드라인 승격 15런~~ · ~~채널 그룹 분해~~ | — | 완료 | ✅ |
| ~~OU 바람 ablation~~ | — | 완료 (60 run) — **한계가 실재하고, 문제 자체의 성질임을 특권 팔로 증명** | ✅ |
| ~~중간보고서 작성~~ | — | 초고·요약본 완료 (학술 문체 v2) | ✅ |
| **1** | **그림 1·2 실제 생성** — 그림 1 = 배달 시간–CEP50 평면(오차막대 CEP50–CEP90), 그림 2 = DR 스윕 3곡선 + 부호 반전점 | 보고서에 현재 **캡션과 축 사양만** 있다 | ~40분 | ❌ |
| **1a** | **분량 결정** — 1.0장이 절대 조건인지 확인 후 절단 항목 선택 | 요약본이 약 2장. 남은 후보($R^2$ 사다리·gi 서술·OU 한계·그림 1)가 전부 필수 지정 항목 | 판단 | ❌ |
| 1b | (선택) 누적창 **지수가중** — 접두 평균은 전 구간 동일 가중 | **OU 한계를 완화하는 유일한 후보.** τ=3~10 구간이 회복될 수 있다 | ~30분 | ❌ |
...
| ~~8b~~ | ~~std 고정 팔~~ | 09-14 완료 — **음성** (CEP50 +18.2%, 반대 방향). Rule 44 확정. SL 초기화 팔은 불필요로 미실행 | 0.6 h | ✅ |
| 8c | (사용자 결정) 행동 공간 잔차 RL | 주 claim에 불필요. RL 자체의 가능성만 묻는 별도 질문 | 수 시간+ | ❌ 보류 |
| **9** | **중간보고서에 RL ablation 한 문단** (exp_032·033) + 그림 1·2 생성 | 주 claim 확정에 따른 서술 | ~1 h | ❌ |

**L1-RL이 8번인 이유.** 결합이 해소되면서 RL의 근거 하나가 사라졌다 — "재적합 필요"는
더 이상 RL의 장점이 아니다. 남는 질문은 종단 보상의 한계 가치 하나이고, 그 답이 어느 쪽이든
논문은 성립한다(음성이면 *"잔차의 최적값은 드리프트 예측이다"* 가 결론이 된다).
게다가 5번(보상 결함)을 안 고치고 돌리면 **보상 해킹을 학습한 run**이 나온다.


## 2026-09-02 기준 우선순위

⚠️ **두 분포 이동은 종류가 다르다.** 축 P = **배율(calibration) 오류**(환산표의 $K_{policy}$),
잔차 on/off = **커버리지 부족**(안 가본 릴리즈 시점). **DAgger는 후자만 고친다.**

| # | 무엇 | 고치는 병 | 비용 |
|---|---|---|---|
| **1** | **게인 무관 특징** — tilt를 "지령 가속 − 실제 가속" 누적으로 교체 → 재적합 → seed 2 주입 | **배율 오류(축 P).** 성공 시 논문 한계 항목 소멸 | ~15분 |
| **2** | **DAgger 1바퀴** (잔차 켠 채 재수집 → 합쳐서 재적합) | **커버리지 부족.** 0.209 → 천장 0.192 방향 | ~30분 |
| **3** | 조준 보상 잔차 포함 결함 수정 | **L1-RL 전 필수** | ~30분 |
| 4 | seed 2 재수집(3,500 ep) 후 재적합 | "라벨 부족 vs 여유 부족" 구분 | ~10분 |
| 5 | 적합 손실에 시간 평활 항 | EMA의 원인 처치 | ~20분 |
| 6 | 3-seed 각각 재적합 → 분산 보고 | **1번 실패 시에만** 필요 | ~40분 |
| 7 | OU(시변) 바람 ablation | tilt 누적의 전제 — 남은 마지막 한계 | ~1시간 |
| 8 | Table 1에서 T1 또는 T2@3.0 제거 · `research_architecture` §7 갱신 | 100 Hz 판정에서 동일해진 arm | ~20분 |

**순서 이유:** 1번이 성공하면 회귀기 입력 설계가 바뀌고 DAgger는 바뀐 입력 위에서 다시
돌려야 한다. 뒤집으면 일을 두 번 한다.

**L1-RL의 위치 (09-02 재조정):** RL은 결합 문제를 없앤 게 아니라 옮긴다 —
SL은 정책 변경 시 몇 분짜리 재적합, RL은 애초에 전체 재학습이라 "전이할 잔차"가 없다.
게다가 RL은 릴리즈 게이트 요동 문제를 더 크게 안는다(탐험 잡음). 검증할 질문은
"RL이 결합을 푸는가"(자명)가 아니라 **"그 대가를 치르고도 SL의 85% 회수를 넘는가"** 이다.
...
- [x] **(Isaac Lab, exp_019)** 물리 페이로드 attach/detach — kinematic weld 구현·검증 완료(4/4 PASS). → [[experiments/legacy/exp_019_physical_payload]]
...
- [x] **(Isaac Lab, exp_015)** Phase별 순차 커리큘럼 **코드 구현 완료** — action 4→6 residual, phase 노브+파생 플래그, 릴리스 이벤트+DR 착탄오차 터미널 보상, Gauss-Markov 이동타겟+lead, `train.py --phases` 오케스트레이터. `py_compile` 12파일 통과. → [[experiments/legacy/exp_015_phased_curriculum]] / [[research/legacy/phased_curriculum]] / Rule 20
- [x] **(Isaac Lab, exp_015)** 본 학습 완주 — `--phases 1,2,3 --phase_iterations 600,500,500 --num_envs 2048`(baseline, ~65 min, ORCH_EXIT=0). Phase 1 완전 수렴(success 1.00), P2/P3는 방향성 신호(drop↓/lead best 0.071m)만·명중 미형성. → [[experiments/legacy/exp_015_phased_curriculum]] §7 / [[research/legacy/curriculum_phase_convergence]]
- [ ] **(Isaac Lab, exp_015 후속)** P2/P3 명중 능력 확보: (a) exp_018 `release_terminal` 구조를 Phase 2·3에 적용(warm-start=exp018 B0 검토) 또는 (b) P2/P3 iteration 대폭 증량. PhaseCfg(residual_scale/release_tolerance/w_impact/w_lead) 튜닝. → [[research/legacy/curriculum_phase_convergence]]
- [ ] **(Isaac Lab migration, 병행 트랙)** L4 Spot VM 기동(`infra/deploy.sh` 빌드+push, `infra/startup.sh` 실행) → Cartpole 스모크 → `Isaac-DroneBombard-Direct-v0` env 스모크(2-iter) → `play.py --zero-actions/--scripted` 물리 검증. → [[experiments/legacy/exp_012_isaac_migration_phase2]]
...
- [ ] **(Isaac Lab migration, 게이트 조건부)** `feat/isaac-env-migration`(이 워크트리)에서 `ros2_ws/`/`gazebo_models`/PX4 파일 정리 — **jekyun(라이브 SAC)는 대상 아님, 절대 미삭제.** 위 2개 항목(env 스모크 통과 + PX4 스텝응답 캡처) 완료 전까지 보류(사용자 확인, 2026-07-03). → [[experiments/legacy/exp_012_isaac_migration_phase2]] §8
...
- [x] **fps 개선 — CRUISE 타임아웃 근본 원인 수정** — arm 게이팅(#2) + early-bail(#4). v12에서 효과 검증 중 → [[research/legacy/cruise_timeout_arming]]
- [x] **v12/v13 arm 처리량 재진단** — `ARM REJECTED` 0회(게이팅 작동), 실제 병목은 EKF 재수렴이 10s bail 초과(13–16s). **arm_bail 10→20s 적용** → [[experiments/legacy/exp_006_xgzum51v_armdiag_dryrun]] / Rule 11
- [x] **v13 재시작** — 30K 체크포인트 인시던트로 소실 → **fresh 재시작**(iyhfy5ps, arm_bail=20). 검증: bail 0, late-EKF 회복 확인.
- [x] **핸드오프 윈도우 확장** — 고도↑(레버 아님) 기각, **탐지 게이트 수정**(conf 0.5 + 공간필터 200→300 px)으로 핸드오프 2.7→5.0 m(~2배), spurious 0. dry-run PASS, **미커밋** → [[experiments/legacy/exp_008_dryrun_alt10_handoff_window]] / Rule 13
- [x] **리셋 처리량 병목 규명 + 수정** — 원인=teleport 후 EKF 재수렴(param 불가). **soft reset로 ~3.9×** 프로토 검증 → [[experiments/legacy/exp_009_softreset_throughput]] / Rule 14
- [x] **soft reset 장기 검증** — byxyaf4d 3096 resets에서 soft 성공 ~91%, EKF bounded, no teleport/no PX4 restart. **Rule 14 검증완료.** → [[experiments/legacy/exp_010_byxyaf4d_v14_195k_eval]]
- [ ] **⚠️ v14 정책 회귀(65%<80%) 해소** — 195K eval 실패 전부 final-approach stagnation(미성숙 가설). 옵션: ① soft reset 켠 채 500K까지 재개/연장(미성숙 검증) ② 10m→5m 고도 A/B(종말 거리 가설) ③ 종말 보상 shaping 강화. → [[experiments/legacy/exp_010_byxyaf4d_v14_195k_eval]]
...
- **2026-09-25:** exp_038 평활 항(λ 5/20/50, 27 run) ✅ EMA 대체 가능 · exp_039 게이트 손실/게이트 가중(9 run) ⛔ 이득 없음 · exp_040 GRU-C 최종 비교(36 run) 🟰 12/12 동률 → 본 방법 = GRU-S + EMA 유지 · exp_041 EMA α 스윕(21 run) ✅ [0.1,0.4] 평탄, α=0.3 확정.

- **2026-09-24:** **exp_037 상대 수축(p2·p4, 1000 epoch × 2) + 현실 바람 평가(8팔 × A·B × 3 seed + 상대 수축 τ 사다리 30 run).** 현실 돌풍 A/B: GRU-S 0.201/0.219(−33%/−28%), 오라클 0.194/0.198. 상대 수축: 정상 0.205~0.207, τ ≤ 3 악화. → [[notes/experiments/exp_037_relative_shrink_realistic_wind]]

- **2026-09-24:** **exp_036 분산 헤드(GRU 26→64→4, NLL, 실현 라벨, 1000 epoch, 주입 15 run).** GRU-U τ=∞ 0.210 / τ=10 0.237(−13.1%) / τ=3 +4.8% / τ=1 +9.0% / τ=0.3 +18.5%; GRU-R 대비 전 τ 개선. → [[notes/experiments/exp_036_uncertainty_head]]

- **2026-09-24:** **exp_035 실현 라벨(GRU 26→64→2, 1000 epoch × 2팔, 주입 30 run).** GRU-R τ=∞ 0.202 / τ=10 0.242(−11.0%) / τ=3 +7.0% / τ=1 +17.3% / τ=0.3 +24.1%; 순간 라벨 대조군보다 전 τ 개선. → [[notes/experiments/exp_035_realised_label]]

- **2026-09-14:** **exp_034 학습된 시간 필터(GRU 26→64→2, PPO 없음) — 45 run.** GRU-S(정상 학습) τ=∞ CEP50 0.204(gi 0.209) → 수제 누적 대체 ✅. 혼합 학습(τ 10/3/1 덤프 2,250 ep 추가) GRU-M/MLP38-M은 τ ≤ 3에서 더 나쁨 → 라벨 병목(Rule 45). → [[notes/experiments/exp_034_learned_temporal_filter]]

- **2026-09-14:** **exp_033 L1-RL 파일럿(std 0.05 고정, `tog7jqs5`) — 110/1000 iter 중단, 음성.** 잔차 0.19 m로 자랐지만 참 드리프트와 반대 방향(R² −0.48, 편향 +0.17 vs −0.086). model_100 paired n=600: CEP50 0.361(+18.2% vs L0), succ@0.5 65.67%. Rule 44 확정. → [[notes/experiments/exp_033_tog7jqs5_l1rl_fixed_std_pilot]]
...
| 날짜 | Run ID | Steps | 요약 |
|------|--------|-------|------|
| 2026-07-21 | **exp019 물리 페이로드 attach/detach (검증 전용, 병행 트랙)** | 0 (8 envs hover-drop 검증, isaac-verify) | **kinematic weld 구현 + 4/4 PASS.** 부착 추적 1.1 mm, 분리 0/8 잔류, 착탄 8/8, 측정 vs 해석적 CCIP \|Δ\| mean 0.012/max 0.021 m. 보상·종단 bit-identical. Rule 24. → [[experiments/legacy/exp_019_physical_payload]] |
| 2026-07-13 | **exp015 이어학습(2차) P2/P3 ext (Isaac PPO, 병행 트랙)** | P2: +2000 iters (1098→3097) · P3: +2000 iters (3097→5096), 2048 envs, ~3h | **iter 예산 확대 검증 — 0.8m 돌파 ❌.** P2 ext: drop 2.87m(정체), release 0.33→0.01, success 0. P3 ext: drop 5.31m(회귀), reward 74.5, success 0. Rule 20f. → [[experiments/legacy/exp_015_phased_curriculum]] §8 |
| 2026-07-12 | **exp015 실학습 (baseline, Isaac PPO, tensorboard, 병행 트랙)** | 2048 envs × 600/500/500 iters (~104M steps, ~65 min) | **Phase 1→2→3 커리큘럼 첫 완주(ORCH_EXIT=0). Phase 1만 완전 수렴.** plain `--phases 1,2,3`. P1 success 0.48→1.00·reward→107(exp_014 재현); P2 reward −0.8→94.7·drop 4.66→2.91m↓·release peak 0.98/tail 0.33·success~0; P3 reward→102·lead best 0.071m·success~0. warm-start 무손실(Rule 20e). reward 우상향=proximity 지배 — P2/P3 명중 미형성(추가 학습 또는 exp_018 종단구조 필요). ckpt/로그/그래프 host 영속화. → [[experiments/legacy/exp_015_phased_curriculum]] §7 / [[research/legacy/curriculum_phase_convergence]] |
...
| 2026-06-22 | dryrun_alt10 (uqy7lmny/_gated, offline) | 1500×2 (dry-run) | **핸드오프 윈도우↑.** 고도만 10 m=실패(레버 아님; 마커 작아짐 + 200 px 필터 캡). 10 m+탐지 게이트(conf 0.5 + 200→300 px)=성공: 핸드오프 2.7→5.0 m(~2배), spurious 0. **미커밋.** → [[experiments/legacy/exp_008_dryrun_alt10_handoff_window]] / [[research/legacy/detection_gate_vs_altitude]] / Rule 13 |
| 2026-06-22 | EKF A/B + softreset proto (offline) | 1500+5000 | **리셋 처리량 ~3.9×.** 원인=teleport 후 EKF 재수렴(`pre_flight_checks_pass=False`). EKF2_GPS_CHECK 0 A/B=음성(COM_ARM_WO_GPS). **soft reset(teleport 회피)=성공**: 0.93→3.61 handoffs/min, fps 2→9, reset 65s→11s, soft 100%, EKF 안정. → [[experiments/legacy/exp_009_softreset_throughput]] / Rule 14 |
| 2026-06-22 | rl_yolo_v14_softreset (byxyaf4d) | 진행 중 (fresh 0→500K) | **full run — soft reset ON 장기 검증.** 10m + 탐지게이트 + soft reset 일괄, online. EKF drift bounded?/fallback율?/실제 ETA(~15h 예상) 확인 후 커밋. → [[experiments/legacy/exp_009_softreset_throughput]] |
| 2026-06-23 | rl_yolo_v14_softreset (byxyaf4d, stop @196.5K) | 196.5K/500K(~39%), reward plateau | **plateau stop + 195K eval = 65%(13/20).** v13 80% 대비 회귀(실패 전부 final-approach stagnation). EKF 귀책 0. **Soft reset 장기검증 ✅**(3096 resets, soft ~91%, EKF bounded) → Rule 14 검증완료. 비디오 3/3 success 캡처. 회귀=미성숙(39% budget). commit 결정 대기. → [[experiments/legacy/exp_010_byxyaf4d_v14_195k_eval]] |
| 2026-07-01 | rl_yolo_v15_bc_stable | 진행 중 (fresh 0→300K) | **wobble 교정(LPF+B+C) 적용 fresh run.** jerk RMS 2.92→1.61(−45%) A/B 확정 후 기동. → [[experiments/legacy/exp_011_wobble_lpf_reward_damping]] / Rule 15 |
| 2026-07-03 | isaac_migration_phase2 (`feat/isaac-env-migration`, 병행 트랙) | 0 (코드만) | **Isaac Lab env+PPO 이식.** v13/v15 parity, `pytest test_math.py` 29/29 통과, L4 VM 미기동. jekyun SAC 학습과 별개 브랜치/워크트리. → [[experiments/legacy/exp_012_isaac_migration_phase2]] |
...
| 2026-07-03 | **exp013_v2_visionfix (wcjklw7a, Isaac PPO, 병행 트랙)** | 65.5M steps (1000 iters 완주) | **첫 완주 + deterministic 200-ep eval = 36%.** plateau @iter 700, d_xy_min 1.4m 정체. 실패: max_alt 33%(상승 farming, Rule 17)+crash 27%. farmer(+225)>finisher(+121) 불균형(Rule 18a), noise_std 0.8→3.92 폭주(Rule 18b). **사후 --zero-actions FAIL(11.9m): 리셋 속도킥 활성 — run 오염, max_alt 1차 용의자.** 다음=exp_014(0순위 킥 수정 → conf 거리감쇠+success 300+entropy 0, fresh). → [[experiments/legacy/exp_013_wcjklw7a_isaac_ppo_first_training]] / [[research/legacy/isaac_ppo_tuning_recommendations]] |
| 2026-07-05 | **exp014 A2 (v3qk07pg) + A0′ (azoc1xp0), Isaac PPO, 병행 트랙** | 각 26.2M steps (400 iters) | **plant 수정 + 비전 거리감쇠 → deterministic 200-ep eval = 100.0% (202/202), d_xy_min 0.665m.** 수정: ①킥→스폰타임 MassAPI authoring(게이트 11.9m FAIL→0.2m PASS) ②로터 스핀 리셋 재주입 제거 ③**inertia 대반전**: `set_inertias`는 전파되고 있었음 — exp_013은 rate loop ~1300× 저토크 plant(Rule 19, 구 정책 plant-overfit로 무효). A2: R_alt=0.0000(climb 창발→기각 시그니처), noise_std 0.80 안정(폭주 없음). A0′ 대조: R_alt 0.0365 → 지배 요인=plant 일관성, 감쇠=꼬리 제거+YOLO parity(유지). 실 YOLO 캘리브레이션은 이미지 annotator 버그로 차단(하네스 수리 완료). reward_success·entropy 불변(다음 페이즈). → [[experiments/legacy/exp_014_A2_visionrange]] / [[research/legacy/isaac_inertia_ctrl_mismatch]] |
| 2026-07-05 | isaac_phased_curriculum (`feat/isaac-env-migration`, 병행 트랙) | 0 (코드만, 미학습) | **Phase별 순차 커리큘럼 구현.** 이미지 3단계(접근/nominal → CCIP+Residual/정지 → 이동타겟) 완전 구현: action 4→6(δ residual), phase 노브+파생 플래그, 릴리스 이벤트(nominal CCIP+δ 트리거 → 실제 DR 낙하 착탄오차 터미널 보상), drag/wind DR, Gauss-Markov 이동타겟+lead, `train.py --phases 1,2,3` 서브프로세스 오케스트레이터(6-dim 고정 → warm-start 무손실). Phase 1=exp_014 baseline 동작 동일. **`py_compile` 12파일 통과; `pytest`(+8 신규)·본 학습은 L4/컨테이너 대기.** → [[experiments/legacy/exp_015_phased_curriculum]] / [[research/legacy/phased_curriculum]] / Rule 20 |
| 2026-07-05 | **exp016 CCIP 릴리스 referee 재평가 (eval-only, A2 ckpt, 병행 트랙)** | 0 (200-ep deterministic eval) | **디커플링 규명: 4.59m = 지표 의미론 버그(릴리스 트리거 부재, 성공-종단 속도 캐리).** CCIP referee(≤0.2m) 수정 후: 발화 시 0.137m, release_rate 6%/11.5%(10/100Hz), aim_err_min med 0.755m ≈ d_xy_min(cross-track 지배). 구 지표 4.649m 재현. 보상/종단 bit-identical. Rule 21. → [[experiments/legacy/exp_016_ccip_release_reeval]] / [[research/legacy/ccip_release_decoupling]] |
| 2026-07-06 | **exp017 Stage A — 밀집 CCIP 조준 보상 (750gpldr/6z0gpnhy/fv5qqmtz, Isaac PPO, 병행 트랙)** | 3 runs (P1 400 + v1 400 + v2 600 iters, 2048 envs) | **보상-변경-단독은 release_rate 못 올림 — 판정 (b), Rule 22.** det 200-ep: 기준선 2.5% → v1(w=1) 5.5%(aim 0.889m·speed 2.72, 방향 실재·p≈0.13) → v2(w=2/knee 1.0) 3.5% 회귀. P1 기준선 학습 내 12→3.7% 단조 하락(근접 최적화가 릴리스 능력 파괴). 원인=γ-할인 완주 보너스·CCIP 노이즈 증폭(×1.53s)·성공 조기 종단. 5-lens 사전 검증, farm 0. ckpt 3종 분리 보존(+호스트 백업) — 차기 warm-start=stageA(v1). → [[experiments/legacy/exp_017_stageA_aim_reward]] / [[research/legacy/ccip_aim_reward_stageA]] |
| 2026-07-06 | **exp018 Stage B — 릴리스-종단 이벤트 (xt0hrr1c/0ns10yso/4vaodj0o/kk06wsbx, Isaac PPO, 병행 트랙)** | 4 runs × 400 iters (전부 v1 warm-start) | **릴리스-종단 구조 → det release_rate 100.00%, drop err 0.125 m (Rule 23).** 종단 교체 단독(B0)으로 5.5%→100%(학습 내 23→99.6% 단조 상승 — Stage A 하락 반전, Rule 22a 인과 확정). aim 보상 노브 불감(w 0/1.5 100%, knee 0.75만 98.5%) — 자동 발화 referee가 노이즈를 +100 샘플러로 전환. done-flag alias 버그 사전 수정(eval success 0% 위험). 호버-드롭 수렴(종단 속도 0.11 m/s). **Stage C warm-start = exp018_B0_final.pt.** → [[experiments/legacy/exp_018_release_terminal]] / [[research/legacy/release_terminal_stageB]] |
| 2026-07-23 | **exp020 물리 페이로드 부착 첫 학습 (o5jn9xzk train / vryuc6mu eval, Isaac PPO, 병행 트랙)** | 400 iters (2048 envs, warm-start=exp018_B0, 보상 bit-match) | **물리 페이로드(kinematic weld) 학습 비용 = 0 — det 200-ep success/release 100.00%, drop err 0.169 m.** `physical_payload=True`가 유일한 델타, release_rate 첫 롤아웃부터 100%(재학습 과도기 없음). σ 드리프트 1.41→1.71 모니터 대상. `play.py --wandb` eval-figure 파이프라인 신설. 컨테이너 빈 WANDB_API_KEY 함정(`--env-file` 필수). ckpt 호스트 `/opt/drone-bombard/checkpoints/exp020/`. → [[experiments/legacy/exp_020_o5jn9xzk_payload_training]] |
| 2026-07-30 | **exp021 v19 + 이동 타겟 CV/CT/CA warm-start 학습 (a6saa42b/29jqq1lu/ntumqwoz, Isaac PPO, 병행 트랙)** | 3 runs × 1000 iters (2048 envs, warm-start=준상 v19 precise 사본) | **이동타겟 모션(CV/CT/CA)을 v19에 obs-보존 포팅 → 3종 학습 완주.** 종반 창: cv release 0.67–0.90/drop 0.39–0.90 m, ct 0.43–0.80/0.24–1.24 m, ca 0.40–0.78/0.32–1.96 m(reward 음수 잔존 — 가속 타겟 최난). 난이도 cv<ct<ca. det eval 후속. ckpt `/opt/drone-bombard/checkpoints/exp021/`. → [[experiments/legacy/exp_021_v19_moving_target]] |
| 2026-07-30 | **exp021 det 200-ep eval 3종 (1nvvuogg/prdqujah/gdow3vfg, eval)** | 0 (deterministic eval only) | **cv success 44.5%/release 81.5%/drop med 0.775 m · ct 33.8%/82.6%/0.783 m · ca 16.5%/63.5%/1.063 m.** 릴리스 이월·명중은 리드 부재로 경계 몰림(released-miss>success). 개선 1순위=리드 개입(KF obs 포팅/target-vel obs/w_lead). play.py --wandb NameError(impacted) 수정, eval seed 미고정 표본 변동(32↔44.5%) 확인. → [[experiments/legacy/exp_021_v19_moving_target]] §3b |
