...
## Rule 39 — 잔차의 누적 특징에서 컨트롤러 게인을 소거하라 (구: 정책마다 재적합)
...
**원인 (2026-09-07 정정).** tilt 누적이 재는 평균 자세는 **두 성분의 합**이다 —
바람을 버티느라 기운 것 $\theta_{wind} \approx F_{wind}/(mg)$ 와 가속하느라 기운 것
$\theta_{man} \approx \bar{a}_{prefix}/g$. 앞쪽은 **힘 균형이라 컨트롤러와 무관**하고
(어떤 컨트롤러든 $mg\tan\theta = F$), 뒤쪽은 **얼마나 공격적으로 가속하는가**에 달렸다.

에피소드를 완주하면 $\bar a \approx 0$이라 뒤쪽이 저절로 상쇄되지만, 잔차가 실제로 쓰는 것은
**접두 평균**이고 그 구간에서는 $\bar{a}_{prefix} = (v_t-v_0)/t \neq 0$ 이다. 회귀기는
seed-1의 가속 프로파일이 섞인 자세로 환산표를 외웠고, 다른 정책은 다르게 가속하므로
**계통적으로 잘못 보정된 이득**이 된다. 잡음이 아니라 편향이라 큰 바람에서 크게 틀린다.

⛔ ~~"정상상태 자세 편향은 컨트롤러 강성 $K$로 결정되므로 $\bar\theta \approx F_{wind}/K$"~~
— 초기 설명이었으나 **틀렸다.** 게인이 정하는 것은 기울기가 아니라 **속도 추종 오차**다.

**정보 문제가 아님의 증거.** ① 오라클은 같은 정책에서 −30.2%를 낸다(주입 경로 정상),
② seed-2의 회귀 $R^2$가 오히려 더 높다(obs+tilt 0.631 vs 0.611), ③ 재적합으로 복구된다.

**처방 (2026-09-07 개정).** ~~정책을 바꾸면 잔차를 무효로 간주하고 다시 적합하라.~~
→ **진짜 처방은 특징을 고치는 것이다.**

누적 자세는 $F_w/K$이고, 여기에 **접두 평균 가속** $(v_t-v_0)/t$ 2채널을 함께 주면
신경망이 $K$를 소거할 수 있다. 에피소드 *전체* 평균이면 $\bar a \approx 0$이라 게인이 저절로
소거되지만, 정책이 들고 다니는 **접두** 평균에서는 그 항이 남아 오염원이 된다.

실측: seed-1 데이터로만 적합한 gi 회귀기를 seed-2 정책에 주입 →
회수율 22.9% → **77.1%** (seed-2 자체 재적합 72.3%보다 높다), CEP90 0.830 → **0.464**.
자기 정책에서도 CEP90 회수율이 64% → **90%**로 오른다.

> **일반화한 규칙: 잔차의 입력에 접두 누적 통계를 쓸 때는 그 통계에 컨트롤러 게인이
> 들어가는지 따져라.** 들어간다면 게인을 소거할 짝 항을 같이 준다.
> 확인하지 않았다면 "배포할 정책의 데이터로 적합하라"가 여전히 유효하다.

⚠️ 접두 평균은 **바람의 정상성을 가정**한다. OU(시변) 바람에서는 미검증이다.

→ [[research/residual_policy_coupling]] §7 · [[experiments/exp_030_gain_invariant_residual]] ·
[[experiments/exp_029_l1_sl_generalization]]
...
## Rule 41 — 잔차 회귀기를 $R^2$만으로 통과시키지 마라

**증상.** 정책 전이 arm의 오프라인 $R^2$는 0.611 → 0.554로 **−0.057**밖에 안 떨어졌는데,
주입하면 오라클 회수율이 85% → **22.9%**로 무너졌다. $R^2$를 봤다면 통과시켰을 것이다.

**원인.** $R^2$는 **분산 설명률**이다. 오보정의 손상은 분산이 아니라 **계통 편향과 꼬리**에
있고, 게다가 릴리즈 게이트가 **첫 교차** 판정이라 예측 오차가 타이밍 편향으로 증폭된다
(Rule 38 / [[research/release_gate_jitter]]). 회귀 지표와 과제 지표 사이에 이 증폭단이 있다.

**처방.** $R^2$는 **정보가 있는가**(관측이 장님인가)를 묻는 데만 쓴다.
**잔차를 채택할지는 반드시 주입해서** CEP50 · CEP90 · succ@1.0로 판정한다.
주입 평가는 3 seed × 200 ep = 약 90초다. 아낄 이유가 없다.

→ [[experiments/exp_030_gain_invariant_residual]] §4.2 · [[research/residual_observability]]

---

## Rule 42 — 결과 공간 잔차는 외란의 준정상성을 전제한다 ($\tau \gtrsim 10\,t_{fall}$)

**증상.** 바람을 OU(상관시간 $\tau$)로 바꾸면 잔차의 이득이 $\tau \approx 10$ s에서 소멸하고
$\tau \lesssim 3$ s에서 **순손실**이 된다(CEP50 +3.5% / +9.8% / +10.7%). 정상분포는 학습
조건과 동일하게 고정했으므로 **세기가 아니라 변화만**의 효과다. 비행은 멀쩡하다 —
L0의 CEP50은 0.305 → 0.272~0.294로 사실상 불변이고 배달률은 95%를 유지한다.

**원인 — 추정기의 한계가 아니다.** 참 순간 바람을 받는 **특권 팔이 같은 지점에서 진다**
($\tau$=3에서 +6.0%, 1에서 +40%, 0.3에서 +92%; 모두 EMA 적용 후). 착탄점은
**낙하 $t_{fall}\approx0.85$ s 구간의 바람 적분**으로 정해지는데 오라클은 릴리즈 순간의
바람만 안다. 나머지는 **미래**다. 필요한 것은 더 나은 관측이 아니라 **예보**다.

**처방.**
- 준정상성이 의심스러우면 **잔차를 끈다** (켜면 순손실 — Rule 40의 세 번째 발현).
- **낙하 시간은 설계 변수다.** 낮게·빠르게 던질수록 $t_{fall}$ 이 줄고 조건이 완화된다.
- ⭐ **상한선 팔에도 학습 팔과 같은 평활을 적용하라.** 원본 오라클은 $\tau=10$ 에서
  잔차 없음보다 나쁘다(+15.8%, CEP90 1.233) — 천장이 지면 회수율 지표 자체가 무의미해진다.
  상수 바람에서 EMA는 오라클을 0.192 → 0.188로 거의 바꾸지 않으므로 부작용도 없다.

→ [[research/residual_wind_stationarity]] · [[experiments/exp_031_ou_wind]] · Rule 38, 40

---

## Rule 43 — 잔차 팔끼리 비교하려면 정보·권한·평활을 먼저 맞춰라; 동결은 "그래디언트 마스킹"으로 끝나지 않는다

**상세:** [[research/l1_rl_preflight]] (2026-09-13)

**증상(사전 감사).** 기존 코드로 L1-RL을 돌리면 L1-SL에 지게 되어 있었다 — RL과 무관한 세 이유로.
① RL 잔차 입력이 관측 26채널뿐인데 SL은 38채널(누적 12채널 추가). **같은 정보량의 SL 팔은 L0에게도
진다**(CEP50 0.305 → 0.329). ② SL은 EMA 0.3을 걸었고 RL은 평활이 없어 첫 교차 게이트 편향(Rule 38)을
RL만 떠안는다. ③ 학습 잔차 권한 1.0 m vs SL 평가 2.0 m로 클램프가 다르다.

**동결의 함정.** `--freeze_nominal`은 출력층 nominal 행의 **그래디언트를 0으로 마스킹**했지만
`runner.load()`가 L0의 Adam 모멘트를 복원하므로 **grad가 0이어도 Adam은 스텝을 밟는다**
(`p.grad is None`일 때만 건너뛴다). 단위테스트는 빈 Adam으로 검사해서 이것을 못 잡았다
(`tests/test_residual_actor.py::test_masked_gradient_alone_does_not_protect_from_adam_momentum`).
또 트렁크를 통째로 얼리면 새 관측 채널을 잔차가 볼 수 없다 — 첫 층이 얼어 있으니까.

**교훈.**
- **비교 팔은 학습 신호 하나만 다르게 하라.** 입력 채널·출력 권한·시간 평활·평가 시드가 다르면
  그 차이가 결과에 섞인다. 누적 채널은 env가 만들어 관측 **맨 뒤에** 붙인다(`accum_obs`, obs[:26] 불변).
- **동결은 옵티마이저 밖으로 빼는 것이다.** 마스킹이 아니라 `requires_grad=False` + 별도 잔차망 +
  옵티마이저 재생성. 복원된 옵티마이저 상태는 동결 파라미터를 움직인다.
- **등가 검사를 수치로 하라.** env 내부 누적 채널 + 38입력 회귀기 주입이 기존 `play.py` 경로와
  16지표 × 200에피소드 전부 동일함을 확인한 뒤에 RL을 걸었다.

---

## Rule 44 — 결과 공간 잔차는 종단 보상 RL로 배울 수 없다: std를 학습시키면 탐험이 소멸하고, 고정하면 릴리즈 트리거를 farming한다

**상세:** [[research/residual_rl_exploration_collapse]] · [[experiments/exp_032_icpj8p4r_l1rl_zero_pilot]] (2026-09-13)

**증상.** 정보(38채널)·권한(2.0 m)·평활(EMA 0.3)·정책(동결 L0)을 L1-SL과 똑같이 맞춘 0 초기화 잔차 PPO가
300 iteration 후 **L0와 구별되지 않았다**(CEP50 −1.9%, n=600; SL은 −31.6%). 잔차 std는 80 iteration 만에
0.05 → 0.01로 바닥에 붙었고, 잔차 크기는 0.05 m에서 220 iteration 평탄. 롤아웃 성공률 79 → 92%는 전부
잡음 제거였다.

**원인.** 첫 교차 게이트가 잔차 잡음을 계통적 조기 릴리즈로 바꾸므로(Rule 38) std를 줄이는 그래디언트가
항상 존재한다. std가 바닥이면 평균의 정책 그래디언트 SNR이 0이다. 지도학습은 라벨이 있어 이 딜레마가 없다.

**교훈.**
- RL 잔차를 돌리려면 **std 고정/하한** + 그 std의 게이트 비용을 먼저 측정(용량-반응: 0.2 → 60%, 0.05 → 80%, 0.001 → 96%+).
- 0에서 출발하는 RL 잔차는 드리프트를 찾기 전에 탐험을 잃는다. RL의 자리는 **드리프트 예측 위의 미세조정**.
- 롤아웃 성공률은 학습 성공의 증거가 아니다 — 결정론적 paired 평가로만 판정 (Rule 29·40의 확장).
- **std 고정도 답이 아니다** (exp_033, 09-14): 잔차가 0.19 m까지 자랐지만 참 드리프트와 **반대 방향**($R^2$ −0.48),
  결정론 CEP50 **+18.2%**. 가설은 시간 비용 farming(첫 교차 지점에서 착지 오차는 2차로 평탄, 시간 절감은 1차).
- 확정 근거: 두 팔(exp_032·033). 행동 공간 잔차 RL은 시험하지 않았으므로 이 규칙은 결과 공간에만 적용한다.
- 논문에는 *"같은 정보·권한·평활 위에서 종단 보상 RL은 L0에 머물거나(−1.9%) L0보다 나빠졌고(+18.2%), 지도 잔차는 −31.6%"* 한 문단.

---

## Rule 45 — 시변 외란에서는 라벨이 병목이다: "순간 바람" 라벨로 시변 데이터를 배우면 오히려 나빠진다

**상세:** [[experiments/exp_034_learned_temporal_filter]] §3.4 · [[research/residual_wind_stationarity]] 부기 (2026-09-14)

**증상.** OU 바람 데이터(τ 10·3·1)를 섞어 잔차 회귀기를 학습했더니, 모델이 수제 누적 MLP든 GRU든 τ ≤ 3에서
정상 데이터만 배운 팔보다 **더** 졌다(τ=1: +9.8% → +20~29%). 정상 바람의 꼬리(CEP90)도 나빠졌다(0.41 → 0.45~0.51).

**원인.** 라벨이 "릴리즈 순간의 바람을 고정하고 적분한 드리프트"다. 시변 데이터로 배우면 회귀기는 순간 바람을 더 빨리
추종하게 되는데, 착지는 낙하 0.85 s의 바람 **적분**이라 τ가 짧을수록 순간 바람은 착지와 무관해진다. 잘 배울수록
참 순간 바람을 받는 오라클의 실패 방식(τ=1에서 +40%)에 수렴한다.

**교훈.**
- 학습 목표는 "지금 관측 가능한 양"이 아니라 **"결과를 결정하는 양"** 이어야 한다. 결과 공간 잔차의 정답은
  실현된 착지 드리프트(낙하 구간의 실제 바람으로 적분)이며, 그때 MSE 최적해가 낙하 적분의 조건부 기대값이 된다.
- 시변 조건을 데이터로만 덮으려 하지 마라. 라벨이 틀리면 데이터가 늘수록 더 정확하게 틀린다.
- 학습된 시간 필터(GRU)는 수제 누적 특징을 대체하지만(정상 바람 −33.3% vs −31.6%), 라벨을 고치기 전에는 시변 바람을 못 푼다.
- **실현 라벨은 필요조건이지 충분조건이 아니다** (exp_035, 09-24): 같은 데이터·모델에서 라벨만 바꾸면 전 τ에서 좋아지고
  τ=10에서 −11.0%(오라클+EMA −14.1%)까지 가지만, τ ≤ 3에서는 여전히 L0보다 나쁘다(+7 / +17 / +24%). 회귀기가 τ를 모르고
  혼합 평균 수축률을 쓰기 때문 → 상태별 불확실성(분산 헤드)으로 "모르면 물러서기"를 배워야 한다.
- **분산 헤드는 작동하되 부분적이다** (exp_036): 같은 데이터·라벨에서 GRU-R을 전 τ에서 이기고 τ=10 −13.1%(오라클+EMA −14.1%),
  τ=1 +9.0%(수제 gi 동률). 관측 이력이 τ를 구분하는 정보가 19%(σ 0.197 → 0.234)뿐이라 τ ≤ 3의 손해는 절반만 줄었다.
  시변 바람의 단일 모델은 GRU-U; 완전한 무해(L0 동률)는 미달로 적는다.

---

## Rule 46 — 돌풍만 두는 OU 사다리는 스트레스 시험이다; 현실 돌풍(평균풍 + 20~30%)에서는 잔차 이득이 유지된다

**상세:** [[experiments/exp_037_relative_shrink_realistic_wind]] (2026-09-24)

**증상.** τ 사다리(평균풍 없이 돌풍만, 세기 = 평균풍 분포)에서 τ ≤ 3 s일 때 모든 잔차 팔이 L0보다 나빴다(exp_034~036).
그런데 비행마다 다른 평균풍 위에 Dryden형 돌풍을 20~30% 얹은 현실 조건(τ 10·3)에서는 GRU-S −33% / −28%, 오라클 −36% / −35%로
정상 바람의 이득이 그대로 유지됐다. L0 자체도 거의 안 변한다(0.301 → 0.305).

**교훈.**
- 시변 외란을 시험할 때는 **평균과 변동을 분리**하라. 변동만 두고 세기를 평균 수준으로 잡으면 현실에 없는 조건을 재는 것이다.
  저고도 10 m·풍속 5 m/s의 Dryden 수평 돌풍은 τ ≈ 13 s, 세기 ≈ 평균의 20%다.
- 스트레스 사다리는 "언제 물러서야 하는가"를 재는 도구로만 쓰고, 방법 선택은 현실 조건에서 하라. 그 기준으로 본 방법은
  가장 단순한 GRU-S이고, 실현 라벨·분산 헤드는 보조 변형이다.
- 분산 헤드의 τ 식별력은 19%뿐이라 상대 기준 수축은 성립하지 않는다(현실 조건 +0.01 m는 잡음 범위, 스트레스 조건은 악화).
