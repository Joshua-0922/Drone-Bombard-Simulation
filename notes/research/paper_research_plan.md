...
3. **핸드오프 랜덤화** (v-track): 순항 속도 U[2,6] m/s · 방위 U[0,2π) · 고도 U[8,12] m · 측면 오프셋 · 초기 자세/각속도 소량.
   → 현행 고정 핸드오프는 일반화 주장을 불가능하게 만든다. **가장 시급한 단일 변경.**
4. **DR 확장**: 페이로드 질량 U[0.08,0.12] kg, 항력계수, 기체 질량 ±5%, 컨트롤러 이득 ±10%, 관측/행동 **에피소드 상수 바이어스**.
5. **$T_\text{deliver}$ 로깅** + 체공시간/반송시간 페널티 cfg 토글(기본 OFF).
