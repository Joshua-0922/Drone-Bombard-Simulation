...
> 파라미터 레퍼런스는 [[research/legacy/isaac_lab_reward_tuning]], 메트릭 해석은
> [[research/legacy/isaac_lab_wandb_guide]] — 이 문서는 그 둘을 잇는 절차서.
...
- 코드는 완성되어 **물리 검증(`verify_one_episode.py`, 무학습 1에피소드) PASS** +
  **256-env·20-iteration PPO dry-run 성공**까지 확인됨 (커밋 `5a6a71b`): reward
  ...
  실제 대시보드에 존재함을 보증한다([[research/legacy/isaac_lab_wandb_guide]] 참조).
...
`Episode_Reward/*`(§[[research/legacy/isaac_lab_wandb_guide]] §3)가 기대한 부호/크기로 나오는지
...
[[research/legacy/isaac_lab_reward_tuning]] §0 참조 — 요약하면:
...
- `--run_name <설명>`을 항상 명시적으로 줄 것 (안 주면 타임스탬프만 찍혀 나중에 뭘
  ...
  `v1_wdist3_success05`.
- 메트릭 읽는 법은 [[research/legacy/isaac_lab_wandb_guide]] — 특히 첫 실 run에서는 §5(rsl_rl
...
- [[research/legacy/isaac_lab_reward_tuning]] — 무엇을 바꿀 수 있는지
- [[research/legacy/isaac_lab_wandb_guide]] — 바꾼 결과를 어떻게 읽는지
...
- [[experiments/legacy/exp_012_isaac_migration_phase2]] — 이 브랜치의 실험 번호 시작점, 현재 상태
