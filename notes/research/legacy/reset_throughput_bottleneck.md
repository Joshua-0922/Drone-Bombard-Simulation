...
> 유일하게 통하는 레버다(검증: ~3.9× 처리량). 검증: [[experiments/legacy/exp_009_softreset_throughput]] · Rule 14
...
32연속 soft reset에서 EKF d_xy 안정(4.5–5.8m, 발산 없음). 상세: [[experiments/legacy/exp_009_softreset_throughput]].
...
- **→ Rule 14 production 검증완료** ([[experiments/legacy/exp_010_byxyaf4d_v14_195k_eval]]).
- (잔여) 11s 중 1 m/s cruise가 지배 → flyback/cruise 속도 상향으로 추가 단축 여지.

## 역링크
- [[experiments/legacy/exp_009_softreset_throughput]] — A/B + 프로토타입 측정
- [[experiments/legacy/exp_010_byxyaf4d_v14_195k_eval]] — 장기 검증(3096 resets) + 195K eval
- [[research/legacy/cruise_timeout_arming]] — 같은 뿌리(EKF 재수렴 bimodal), arm_bail 증상 완화 (Rule 11)
