...
> **이 문서의 역할:** [[research/legacy/research_overview_for_paper]]가 *"지금까지 무엇을 했는가"*의 사료라면,
> 이 문서는 *"논문 한 편을 위해 앞으로 무엇을 어떻게 바꿔서 무엇을 학습시키고 무엇을 비교하는가"*의 설계도다.
> 시행착오(인프라·드라이버·arming·EKF·컨테이너)는 여기서 **전부 버린다**(§2b).
> 문헌 조사(105편 + 2편 정독) 근거는 §1. 허브: [[00_index]] · [[research/legacy/research_overview_for_paper]]
...
4. ~~rule-based 베이스라인 부재~~ → **해결.** T0~T3 구현·실측 완료 (§4 Table 1, [[experiments/legacy/exp_023_table1_baselines]]).
...
**1차 실측 (2026-08-03, V19 분포, paired 200 ep, seed 1000 — [[experiments/legacy/exp_023_table1_baselines]]):**
...
2. ✅ **eval 하네스 확장** (`isaac_lab/eval_harness.py`): **paired 평가**(num_envs=episodes로 돌려 각 env 슬롯의
   *첫* 에피소드만 채점 → 첫 리셋은 정책 독립이므로 모든 arm이 bit-identical 시나리오를 봄), CEP50/90,
   ...
   학습 arm(`play.py --paired_eval`)과 규칙 arm이 **같은 코드 경로**를 쓴다. → [[experiments/legacy/exp_023_table1_baselines]]
3. ✅ **핸드오프 랜덤화** (v-track): 방위 ±180° · 속도 U[2,6] · 고도 U[8,12] · 진입 오프셋(횡 ±3/종 ±2) ·
   ...
   → [[experiments/legacy/exp_022_p0_handoff_dyn_dr]]
...
> (c) 둘을 같은 예산으로 비교해 ablation 행으로 승격.
>
> **(a) 실행 결과 = 실패 (2026-08-03, [[experiments/legacy/exp_024_v20_warmstart_failure]]):** v19 warm-start로
> 1000 iter 학습했으나 정책이 **과제가 아니라 페널티 회피로 수렴** — 보상은 −44.8→+10.8로 올랐지만
> 접근/조준/발화율 3종이 전부 평탄하고 상승분은 `out_of_range` 회피뿐. σ 2.10→6.27 폭주 + 액션 포화 93%.
> det eval: v20 **13.0%**(7.5%에서 CI 겹침), **v19 100.00% → 8.50%**(catastrophic forgetting).
> → **관측 프레임 문제는 분포를 넓히는 것만으로 고쳐지지 않는다**는 확증. 남은 경로는 **(a′) 방위 커리큘럼**
> (±30°→±90°→±180° warm-start 체인, v18 데드락을 푼 것과 같은 패턴) 또는 **(b) 관측 프레임 불변화**.
...
- [[research/legacy/research_overview_for_paper]] — 계보·warm-start 체인·F1~F12 원사료
- [[research/rl_rules]] — Rule 19/20/21/22/23/24 (방법론 교훈)
- [[experiments/legacy/exp_017_stageA_aim_reward]] · [[experiments/legacy/exp_018_release_terminal]] — Table 3의 원 실험
- [[experiments/legacy/exp_019_physical_payload]] · [[experiments/legacy/exp_020_o5jn9xzk_payload_training]] — F8
- [[experiments/legacy/exp_021_v19_moving_target]] — Table 7의 출발점
- [[research/legacy/ccip_release_decoupling]] · [[research/legacy/moving_target_models]] · [[research/legacy/phase1_plan]] (baseline_ccip 설계 원안)
