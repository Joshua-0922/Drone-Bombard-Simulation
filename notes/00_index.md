...
- **⚠️ 새로 열린 부채 — 그림 4를 다시 그려야 한다.**
  ...
  튜닝 baseline 상대로는 **CEP50 격차가 L0에서 단조 증가하지 않는다**(−0.023 → −0.093).
  살아남는 형태 둘: ① 잔차를 넣으면 부호가 뒤집힌다(+0.073 / +0.071) → 주 주장은 **L1-SL로** 쓴다,
  ② 성공률·배달률 축에서는 격차가 실제로 커진다(배달률 +16.2 → **+28.2 pp**).
  → DR {0, 0.5, 1.0} 27런 측정 중. [[research/research_architecture]] §7.4b.1
