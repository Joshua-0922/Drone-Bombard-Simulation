...
- **⭐⭐ L0 오차 예산 실측 — 바람이 최대 단일 원인이지만 지표가 포화되어 잔차가 살 것이 없다: [[research/error_budget_l0]]**
  - 바람 상관 $r=0.495$ — **L0는 바람을 흡수하지 못했다.** 바람 성분 RMS **0.255 m** / 전체 0.415 m. 릴리즈 지연 편차 0.186 · 미설명(계통) 0.166 · 조준 0.101 · 탄도계수 ≈0
  - **바람을 완전히 제거해도 `success@1.0`은 92.0 → 93.5%(+1.5 pp)** — n=200의 CI(±5 pp)보다 작아 **원리적으로 측정 불가**. 같은 반사실이 **CEP50은 −33%**(0.319 → 0.214)
  - ⭐ **권고: 헤드라인 정밀도 지표를 CEP50/CEP90으로, `success@1.0`은 배달률/안전 지표로 강등.** 성공 반경은 건드리지 않고 곡선 전체를 싣는다
  - ⚠️ **잔차가 줄일 수 있는 것은 계통 오차뿐** — 릴리즈 지연 편차·탄도계수는 평균 0의 에피소드별 잡음이라 잔차로 안 줄어든다
- **⛔🔧 투하 속도를 잘못된 시점에서 재고 있었다 (정정): [[research/error_budget_l0]] §4**
  - `eval_harness.final_speed_xy`는 **에피소드 종료(페이로드 착지) 시점** 값 — 투하보다 약 1.0 s 뒤다. 드론은 투하 *후에* 감속한다
  - 투하 순간 래치 실측: **v_xy 3.42 m/s · 고도 3.50 m · 전진거리 3.22 m → AeroThrow 분류상 `throw`**. 종전 보고 1.39 m/s는 무효
  - ⛔ 이에 따라 08-30 일지 §8.2 **"빠른 접근 + 감속 + 느린 투하"와 "명시적 투하속도 보상항 필요"는 철회**. 오히려 **논문 주 주장(속도 유지)을 직접 뒷받침**
  - 부수 발견: 정책이 릴리즈 밴드 **바닥(3.5 m)** 에 붙어 던진다 — $t_{fall}$을 줄여 바람 노출과 지연 오차를 함께 줄이는 자발적 전략
- **⭐ seen / unseen × 3 seed 비교 진행 중: [[experiments/exp_027_seen_unseen_3seed]]** (L0 · T0 hover · T1 · T2@1.5 · T2@3.0 · T3 참값 × {학습분포 / 사거리 26–30 m / DR 2.5} × seed {1000,2000,3000}) + L0 학습 seed 2·3 추가 학습
  - 예비: 사거리 외삽(26–30 m)에서 L0 92.0 → **70.5%**, 그런데 **CEP50은 0.317로 불변** — 조준은 완전히 일반화되고 **무너지는 것은 비행(추락 28.5%)** 이다
