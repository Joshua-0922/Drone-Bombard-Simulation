...
### ⭐ 오늘의 결론 — 게이트 인지 학습은 기존 방법과 동률. **본 방법 = GRU-S + EMA(α=0.3, 민감도 스윕으로 확정)**, 평활 항은 논문·코드에서 제외

- exp_041: α ∈ {0.1…1.0} 스윕 → 0.1~0.4 평탄, 0.5부터 꼬리 악화, 무필터 유의하게 나쁨. α=0.3 = 평탄 구간의 최단 지연. 심사자 질문 "α 민감도" 선제 해결.
...
- exp_040: λ=20으로 1000 epoch 재학습한 GRU-C를 논문 표 12조건 전부에서 비교 → 12/12 동률(paired CI 0 포함). 극적 개선 없음 → 사전 기준대로 **GRU-S + EMA 유지**, 표·그림 변경 없음. 평활 항은 본 방법의 부품이 아니므로 ablation도 아님 → 논문·코드에서 제외(기록만 exp_038~040). 다음 = sim2real.
...
- ⭐⭐ **exp_037 현실 바람**: 평균풍 + Dryden형 돌풍(20~30%, τ 10·3)에서 GRU-S −33% / −28%, 오라클 −36% / −35% — **현실 돌풍에서는 이득이 유지된다.** τ 사다리의 손해는 돌풍만 둔 스트레스 시험. 상대 수축은 불채택. **본 방법 = GRU-S**, 분산 헤드는 스트레스 조건 보조 변형.
- **exp_036 분산 헤드**: GRU-R을 전 τ에서 이김 — τ=10 **0.237 (−13.1%)**, 정상 0.210, τ=1 +9.0%(gi 동률), τ=0.3 +18.5%. 분산의 τ 식별력이 19%뿐이라 τ ≤ 3 손해는 절반으로만. 시변 바람용 단일 모델 후보 = **GRU-U**. 후속: 상대 분산 수축(재학습 없이 내보내기 수식만).
...
- **⭐⭐⭐ 정책 결합 해소: [[experiments/exp_030_gain_invariant_residual]]**
  09-02에 발견한 "잔차가 기저 정책에 결합된다"는 한계가 **접두 평균 가속 2채널**로 사라졌다.
  seed-2 정책의 데이터를 **한 프레임도 안 보고** 오라클 회수율 **22.9% → 77.1%**
  (seed-2 자체 데이터 재적합 72.3%보다 높다). **CEP90 0.830 → 0.464.**
  자기 정책에서도 손해가 없고 **꼬리 회수율 64% → 90%**.
  → **논문 Limitations에서 이 항목을 뺀다.** [[research/residual_policy_coupling]] §7
- **⭐⭐ Table 1 시드 정렬 — 철회했던 파레토 지배가 잔차와 함께 돌아왔다**
  exp_027(무학습, 시드 1000/2000/3000)과 exp_028(L1-SL, 3000/4000/5000)이 시드 하나만
  겹쳐 **T2 행과 L1-SL 행이 같은 시나리오 위에서 비교된 적이 없었다.** 무학습 팔을
  4000·5000에서 재비행해 모든 행을 동일한 600 시나리오로 맞췄다(23 run, FAIL 0).
  결과: L1-SL이 튜닝 T2를 **전 축에서** 이긴다 — CEP50 0.209 vs 0.282(−26%),
  CEP90 0.464 vs 0.656, succ@0.5 86.50 vs 65.00%, 배달 5.83 vs 8.05 s, 배달률 94.7 vs 78.5%.
  **L0 단독으로는 여전히 CEP50에서 진다(0.305)** — 차이를 만든 것이 정확히 잔차다.
  → [[research/research_architecture]] §7.4
- **⚠️ 새 규칙 (Rule 41) — $R^2$로 잔차를 통과시키지 마라.**
  전이 arm의 오프라인 $R^2$는 0.611 → 0.554로 **−0.057**밖에 안 떨어졌는데
  주입 회수율은 85% → **22.9%**로 무너졌다. 손상이 분산이 아니라 **계통 편향·꼬리**에 있고
  첫 교차 게이트가 그것을 증폭한다. **채택 판정은 반드시 주입해서** 한다(3 seed × 200 ep ≈ 90초).
- **⭐⭐⭐ 그림 4 확보 — 잔차의 용량-반응 곡선이 0 용량에서 음수를 지난다**
  주 주장의 DR 스윕이 **투하 고도 미튜닝(6.0 m) T2** 상대로 그려져 있어 27런 재측정했다.
  튜닝 baseline 상대로도 CEP50 격차가 **단조 증가**하고 **DR ≈ 0.8에서 부호 전환**
  (−0.059 → −0.035 → +0.020 → +0.073 → +0.071). 단 **L0가 아니라 L1-SL로** 써야 한다.
  ⭐ 그리고 **잔차 자체가 DR 0에서 해롭다** — L0 0.125 → L1-SL 0.141 (**+13.3%**),
  DR 0.5 +6.7%, 1.0 −15.9%, 1.5 −31.4%, 2.5 −32.9%. **보정할 편향이 없으면 분산만 더한다.**
  *"우리 방법이 낫다"* 가 아니라 **용량-반응 곡선**이다 — 인과 귀속의 가장 강한 형태.
  → [[research/research_architecture]] §7.4b.2
...
- **📝 중간보고서 Isaac Lab 파트 개요 확정: [[research/interim_report_outline]]**
  절 순서(Intro→Gazebo→Rewards→**Isaac**→Vision) · 2.1 방법 / 2.2 실험설명(DR 포함) /
  2.3 결과(학습진행·Table 1·일반화) / 2.4 분석·ablation / 2.5 한계(OU).
  ⚠️ 사실 정정 하나 — **"동결 후 2단계 학습"은 구현된 적이 없다.** L0는 단일 단계 PPO.

### 🔧 코드 (2026-09-07)

- `_fit_sl_residual.py` — `--gi`(접두 평균 가속 2채널, 36→38 입력) · `--test_npz`(파일 단위
  홀드아웃 = **오프라인 전이 시험**, GPU 0분)
...
### 연구 (research/)
- [[research/code_map]] — 🗺 **(09-25) 코드 지도 — τ의 뜻, 바람 모델(정상/돌풍만/현실)이 만들어지는 함수·줄, L0 강화학습·GRU 지도학습·평가 코드 위치, 결과 JSON 경로**
...
- [[research/research_architecture]] — ⭐ **(09-07 v5) 최종 아키텍처 — Table 1 시드 정렬판 · L1-SL 경로(§7.6) · 지표/ablation/그림 갱신 · 그림 4 부채(§7.4b.1)**
...
### 실험 (experiments/)
- [[experiments/exp_041_ema_alpha_sweep]] — ✅ **(09-25) EMA α 민감도: 0.1~0.4 평탄, 무필터만 유의하게 나쁨 → α=0.3 확정(데이터 근거 확보)**
