...
> ⚠️⚠️ **2026-08-27 사후 정정 — `release.alt_max` 8 → 12 변경은 실제로 적용되지 않았다.**
> 그 편집을 담은 백그라운드 명령을 중간에 중단시켰고, 파이썬 스크립트가 파일을
> 마지막에 한 번에 쓰는 구조라 **아무것도 기록되지 않았다.** 같은 스크립트에 있던
> `w_ccip` 0.5 → 20도 마찬가지로 미적용이다.
>
> **따라서 아래 측정은 전부 `alt_max = 8.0`, `w_ccip = 0.5`에서 얻은 값이다.**
> 측정값 자체는 유효하고 서로 일관되며, 부트스트랩을 뚫은 것은
> **`w_alt_band` 엔벨로프 셰이핑 하나**였다 — 밴드를 넓힌 것이 아니라
> **밴드로 유도한 것**이 해법이었다.
>
> 부수 결론: `w_ccip`을 20으로 올릴 필요가 없었다. 0.5에서도 정책은 조준 오차를
> 0.25 m까지 줄였다 — 종단 보상과 접근 보상만으로 충분했다.
