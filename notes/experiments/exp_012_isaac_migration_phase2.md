...
- L4 VM 스모크(Cartpole → env 2-iter → zero-actions/scripted/step-response → 2048-env
  throughput)는 **미실행** — VM 미기동. `isaac_lab/README.md`에 정확한 커맨드 문서화.
...
- [ ] L4 Spot VM 기동 (`infra/deploy.sh` 빌드+push, `infra/startup.sh` 실행)
- [ ] Cartpole → env 스모크 → zero-actions/scripted 물리 검증
