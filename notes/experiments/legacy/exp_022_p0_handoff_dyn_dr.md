...
> [[research/legacy/paper_research_plan]] §5 **P0-3 / P0-4 / P0-5(일부)** 실행.
...
## 3. 판정 — v19 정책의 분포 전이 (paired 200 ep, seed 1000, num_envs 200)

> 아래 표는 **2026-08-03 재측정본**이다. 최초 측정(비-paired, seed 42)은
> [[errors/err_20260803_payload_landing_latch]]의 착지 래치 버그가 살아 있던 상태였고,
> 그 버그는 성공을 타임아웃으로 오분류했다. 결론(순서·크기)은 동일하지만 절대 수치가 달라졌다.

같은 체크포인트(`v19_warmstart/precise/model_best.pt`), **동일 200개 시나리오**(paired), 조건만 다름:

| # | 평가 조건 | success (95% CI) | delivery | CEP50 / CEP90 | T_deliver med | 지배 실패 |
|---|---|---|---|---|---|---|
| A | v19 (고정 핸드오프, DR 없음) — 기준 | **100.00%** (98.1–100) | 100% | 0.370 / 0.679 m | 5.95 s | — |
| B | + 동역학/센싱 DR만 (핸드오프 고정) | **99.00%** (96.4–99.7) | 99.5% | 0.408 / 0.680 m | 5.80 s | crash 1 |
| C | + 핸드오프 DR (**방위는 +X 고정**) | **87.50%** (82.2–91.4) | 92.5% | 0.375 / 0.745 m | 6.00 s | bad_att 11 · timeout 4 |
| D | + 전체 v20 (방위 ±180°) | **7.50%** (4.6–12.0) | 8.5% | 0.525 / 1.252 m | 6.50 s | **out_of_range 120(60%)** · bad_att 31 · timeout 30 |

**세 줄 결론:**

1. **동역학/센싱 DR은 사실상 공짜다** (A→B: 100.0 → 99.0%, CEP 0.370 → 0.408 m).
   질량신념 ±5% / 게인 ±10% / 탄도계수 ±20% / 두-시간척도 노이즈를 한꺼번에 켜도 기존 정책이 버틴다
   → **커리큘럼 없이 바로 채택 가능**.
2. **속도·고도·오프셋·자세 랜덤화는 회복 가능한 격차다** (B→C: −11.5 pp). **CEP는 오히려 불변**
   (0.408 → 0.375 m) — 즉 실패는 정확도가 아니라 자세 이탈(bad_attitude 11)에서 온다.
3. **월드프레임 방위 랜덤화는 파괴적이다** (C→D: 87.5 → 7.5%, OOR 60%).
   정책이 표적에 **접근조차 하지 않는다** — "+X로 날아라"를 외운 것.

**투하 스킬 자체는 모든 조건에서 살아 있다**: 발화만 하면 CEP50이 0.37–0.53 m로 유지된다.
...
## 6. 부기 — 기준선 수치 불일치는 선택 편향이 아니라 버그였다

최초 측정에서 동일 `model_best.pt`가 준상 노트의 100%와 달리 **91.0%**로 나왔다. 원인을 추적한 결과
**선택 편향이 아니라 물리 페이로드 착지 래치 버그**였다: 정지한 페이로드는 판정 평면을 영원히
통과하지 못해 성공이 타임아웃으로 오분류되고 있었다(17/200). 수정 후 정확히 **100.00%**로 재현.
→ [[errors/err_20260803_payload_landing_latch]] · [[research/rl_rules]] Rule 28.

[[research/legacy/paper_research_plan]] §2c-2(선택 편향)는 여전히 **미해결**이며, P1-8의 held-out 재평가로
따로 검증해야 한다 — 이번 건은 그 항목의 증거가 아니었다.
...
- eval JSON: 호스트 `/opt/drone-bombard/eval/p0/{T5_policy_v19,T5_policy_v20,condB_dynonly,condC_noheading}.json`

## 관련

- [[research/legacy/paper_research_plan]] — P0 정의, Table 5(충실도 사다리)의 DR 축
- [[research/handoff_generalization_p0]] — 본 실험의 발견 노트 (Rule 27)
- [[research/legacy/research_overview_for_paper]] — §7 방법론 약점 7번
